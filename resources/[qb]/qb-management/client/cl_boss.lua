...
RegisterNetEvent('qb-bossmenu:client:OpenMenu', function()
    ...
    local bossMenu = {
        {
            header = "Patron transactions - " .. string.upper(PlayerJob.label),
            icon = "fa-solid fa-circle-info",
            isMenuHeader = true,
        },
        {
            header = "Manage employees",
            txt = "Check your employee list",
    ...
    QBCore.Functions.TriggerCallback('qb-bossmenu:getplayers', function(players)
        for _, v in pairs(players) do
            if v and v ~= PlayerId() then
                HireMenu[#HireMenu + 1] = {
                    header = v.name,
                    txt = "Citizenship number: " .. v.citizenid .. " - ID: " .. v.sourceplayer,
    ...
    QBCore.Functions.TriggerCallback('qb-bossmenu:server:GetAccount', function(cb)
        local SocietyMenu = {
            ...
            },
            {
                header = "Take money",
                icon = "fa-solid fa-money-bill-transfer",
                txt = "Withdrawal from the account",
                params = {
