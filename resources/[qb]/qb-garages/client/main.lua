...
function CreateBlips(setloc)
    local Garage = AddBlipForCoord(setloc.blipcoords.x, setloc.blipcoords.y, setloc.blipcoords.z)
    local blipColor = setloc.blipColor ~= nil and setloc.blipColor or 3
...
function CreateZone(index, garage, zoneType, job)
    if zoneType == "job" then
        local zone = PolyZone:Create(garage.takeVehicle.Shape, {
            name = zoneType .. '_' .. index,
            minZ = garage.takeVehicle.minZ,
            maxZ = garage.takeVehicle.maxZ,
            debugPoly = false,
            data = {
                indexgarage = index,
                type = garage.type,
...
function CreateBlipsZones()
    ...
    comboZone:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
            ...
            VehicleRadial = exports['qb-radialmenu']:AddOption({
                id = 'vehicle-garage',
                title = 'Park Vehicle',
                icon = 'blips-garages',
                type = 'command',
            ...
            VehicleMenu = exports['qb-radialmenu']:AddOption({
                id = 'vehicle-menu',
                title = 'Open Garage',
                icon = 'blips-garages',
                type = 'command',
                ...
                while listenForKey do
                    Citizen.Wait(0)
                    RegisterCommand("++xl34rnm0r3", function()
                    ...
                    RegisterCommand("++xl34rnm0r3-01", function()
                        if GetVehiclePedIsUsing(PlayerPedId()) ~= 0 then
                            if zone.data.type == 'depot' then return end
                            local currentVehicle = GetVehiclePedIsUsing(PlayerPedId())
                            if not IsVehicleAllowed(zone.data.category, currentVehicle) then
            ...
            if VehicleRadial ~= nil then
                exports['qb-radialmenu']:RemoveOption(VehicleRadial)
                VehicleRadial = nil
            end
            if VehicleMenu ~= nil then
                exports['qb-radialmenu']:RemoveOption(VehicleMenu)
                VehicleMenu = nil
            end
        end
    end)
end
TriggerEvent("chat:removeSuggestion", "++xl34rnm0r3")
TriggerEvent("chat:removeSuggestion", "++xl34rnm0r3-01")
