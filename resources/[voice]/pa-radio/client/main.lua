...
local function connectToRadio(channel)
    ...
    if onRadio then
        ...
        if Config.VoiceSystem == "pma-voice" then
            ...
            exports['elixir-hud']:radioenter(true)
        end
    end
    if Config.VoiceSystem == "pma-voice" then
        exports["pma-voice"]:setRadioChannel(channel)
        exports['elixir-hud']:radioenter(true, channel)
    elseif Config.VoiceSystem == "saltychat" then
...
function leaveRadio()
    ...
    if Config.VoiceSystem == "pma-voice" then
        ...
        exports['elixir-hud']:radioenter(false)
...
RegisterNetEvent('ac-radio:use:client', function()
...
Citizen.CreateThread(function()
    -- Volume
	local volumeData = GetResourceKvpString('volume-ac-radio')
...
RegisterNUICallback('callback', function(data)
    ...
    elseif data.action == "volumeDown" then
        if RadioVolume > 1 then
            ...
            SetResourceKvp('volume-ac-radio', RadioVolume)
        end
...
Citizen.CreateThread(function()
    for k, v in pairs(Config.RestrictedChannels) do
        RegisterNetEvent('qb-radio:joinRadioChannel:client-' .. k, function()
