...
local function getMainDoor(propertyId, doorIndex, isShell)
    ...
    if DoorResource == 'ox' then
        -- Get all doors from ox_doorlock and find the one with the matching id
        local allDoors = exports.ox_doorlock:getAllDoors()
        for _, door in pairs(allDoors) do
            if door.name == id then
                return door
            end
        end
        print("ox_doorlock: No door found with ID " .. id)
        return nil
    elseif DoorResource == 'qb' then
        return exports['qb-doorlock']:getDoor(id)
    end
