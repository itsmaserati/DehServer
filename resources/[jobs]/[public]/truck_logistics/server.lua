...
AddEventHandler("truck_logistics:startContract", function(data)
    ...
    if debug_cooldown[source] == nil then
        ...
        if user_id then
            local contract = exports['oxmysql']:execute('SELECT *, CAST(fragile AS SIGNED) as fragile, CAST(valuable AS SIGNED) as valuable, CAST(fast AS SIGNED) as fast FROM `trucker_available_contracts` WHERE contract_id = ?', { id }, function(result)
                if result and #result > 0 then
                    contract = result[1]

                    print("Contract Details:")
                    print("  Contract Type:", contract.contract_type)
                    print("  Contract Name:", contract.contract_name)
                    print("  Cargo Type:", contract.cargo_type)
                    print("  Fragile:", contract.fragile)
                    print("  Valuable:", contract.valuable)
                    print("  Fast:", contract.fast)
                    print("  Requires Trailer:", contract.trailer)

                    local truckerSkills = exports['oxmysql']:execute('SELECT * FROM `trucker_users` WHERE user_id = ?', { user_id }, function(truckerResult)
                        if truckerResult and #truckerResult > 0 then
                            truckerSkills = truckerResult[1]

                            print("Trucker's skill levels:")
                            print("  Product Type:", truckerSkills.product_type)
                            print("  Fragile:", truckerSkills.fragile)
                            print("  Valuable:", truckerSkills.valuable)
                            print("  Fast:", truckerSkills.fast)

                            if tonumber(truckerSkills.product_type) >= tonumber(contract.cargo_type) and
                                tonumber(truckerSkills.fragile) >= tonumber(contract.fragile) and
                                tonumber(truckerSkills.valuable) >= tonumber(contract.valuable) and
                                tonumber(truckerSkills.fast) >= tonumber(contract.fast) then

                                print("Trucker has required skills for the contract.")

                                if Config.habilidade_distancia[truckerSkills.distance] >= distance then
                                    print("Trucker has required distance skill for the contract.")

                                    if tonumber(contract.contract_type) == 0 then
                                        -- Start Work
                                        TriggerClientEvent("truck_logistics:startContract", source, contract, distance, reward, {})
                                    else
                                        -- Check if you have a truck
                                        local truck = exports['oxmysql']:execute('SELECT * FROM `trucker_trucks` WHERE driver = 0 AND user_id = ?', { user_id }, function(truckResult)
                                            if truckResult and #truckResult > 0 then
                                                truck = truckResult[1]
                                                TriggerClientEvent("truck_logistics:startContract", source, contract, distance, reward, truck)
                                            else
                            ...
                            TriggerClientEvent("QBCore:Notify", source, "Error fetching trucker skills", "error")
                        end
                    end)
                else
                    TriggerClientEvent("QBCore:Notify", source, "Invalid contract data", "error")
                end
            end)
        end
