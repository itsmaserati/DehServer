...
local function RunWorkLoop()
    CreateThread(function()
        ...
        while listen do
            ...
            local TruckCoords = GetOffsetFromEntityInWorldCoords(securityVehicle, -2.0, 0.0, 0.0)
            local TruckDist = #(pos - TruckCoords)

            if (Distance < 15 or hasBag) and DoesEntityExist(securityVehicle) then
                -- If the player doesn't have the bag yet
                if not hasBag and canTakeBag then
                    ...
                    elseif Distance >= 10 and GarbText then
                        GarbText = false
                        exports['qb-core']:HideText()
                    end
                else
                    -- Player has the bag and is near the truck
                    if TruckDist < 6 then
                        if not GarbText then
                            GarbText = true
                        ...
                        if IsControlJustPressed(0, 51) and hasBag then
                            ...
                            QBCore.Functions.Progressbar("deliverbag", Lang:t("info.progressbar"), 2000, false, true, {
                                ...
                                -- After depositing, check if more bags need to be delivered
                                hasBag = false
                                canTakeBag = false
                                DetachEntity(securityObject, 1, false)
                                DeleteObject(securityObject)
                                ...
                                if (amountOfBags - 1) <= 0 then
                                    -- Go to next stop if all bags are delivered
                                    QBCore.Functions.TriggerCallback('securityjob:server:NextStop', function(hasMoreStops, nextStop, newBagAmount)
                                        if hasMoreStops and nextStop ~= 0 then
                                            currentStop = nextStop
                                            currentStopNum = currentStopNum + 1
                                    ...
                                    -- If there are more bags to deliver at the current stop
                                    amountOfBags = amountOfBags - 1
                                    if amountOfBags > 1 then
                                        QBCore.Functions.Notify(Lang:t("info.bags_left", { value = amountOfBags }))
                                    else
                    ...
                    elseif TruckDist >= 6 and GarbText then
                        GarbText = false
                        exports['qb-core']:HideText()
                    end
                end
