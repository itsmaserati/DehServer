...
-- Ensure ped is always spawned, regardless of player job.
local function SpawnPizzaBoss()
...
local function SetupBlipAndTarget()
        ...
        options = {
            {
                ...
                label = "Start Work",
                action = function()
                    if PlayerJob.name ~= "pizzadelivery" then
                        QBCore.Functions.Notify("You don't work here. Visit city hall to apply for a job.", "error")
                    else
                        PullOutVehicle() -- Only start work if they have the job
                    end
                end,
                canInteract = function()
                    return not Hired
    ...
    -- Show the blip only if the player has the job
    if PlayerJob.name == "pizzadelivery" then
