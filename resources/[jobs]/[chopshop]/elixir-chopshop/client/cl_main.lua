...
Citizen.CreateThread(function()
    while true do
        ...
        if scrapblip then
            ...
            if not mert then
                print(scanner)
                exports['elixir-chopshop']:SetupDigiScanner(scanner, {})
                mert = true
            end
            if dist <= 3 then
                if not copsCalled then
                    ...
                    TriggerServerEvent("elixir-chopshop:server:callCops", "Chopshop", 0, streetLabel, pos)
                    copsCalled = true
                end
                Citizen.Wait(5000)
                QBCore.Functions.Notify(Config.Locale["FoundVeh"], 'success')
                exports["elixir-ui"]:Show(Config.Locale["title"], Config.Locale["chop3"])
                Citizen.Wait(10000)
...
RegisterNetEvent('elixir-chopshop:jobaccept')
AddEventHandler('elixir-chopshop:jobaccept', function()
...
function SpawnVehicle(vehicle, x, y, z, w)
    ...
    QBCore.Functions.SpawnVehicle(vehicle, function(veh)
        ...
        exports["elixir-ui"]:Show(Config.Locale["title"], Config.Locale["chop1"])
        ...
        if Config.Email == true then
            ...
            exports["elixir-ui"]:Show(Config.Locale["title"],
...
function ScrapVehicle()
    ...
    if GetVehicleNumberPlateText(veh) ~= LicensePlate then
        ...
        exports["elixir-ui"]:Show(Config.Locale["title"], Config.Locale["chop4"])
        StartChopping()
        DeleteBlip()
    end
end
...
RegisterNetEvent('elixir-chopshop:client:robberyCall')
AddEventHandler('elixir-chopshop:client:robberyCall', function(type, key, streetLabel, coords)
...
function StartAnimation(k)
    if Config.CarTable[k].anim == "wheel1" or Config.CarTable[k].anim == "wheel2" or Config.CarTable[k].anim == "wheel3" or Config.CarTable[k].anim == "wheel4" then
        TriggerEvent('elixir-chopshop:wheelanimation')
        Citizen.Wait(7000)
        ...
        TriggerServerEvent('elixir-chopshop:server:rewardplayer', Config.CarTable[k].anim)
    elseif Config.CarTable[k].anim == "door" then
        TaskOpenVehicleDoor(GetPlayerPed(-1), vehicle, 3000, Config.CarTable[k].getin, 10)
        Citizen.Wait(2500)
        TriggerEvent('elixir-chopshop:dooranimation')
        Citizen.Wait(4200)
    ...
    elseif Config.CarTable[k].anim == "trunk" then
        ...
        TriggerEvent('elixir-chopshop:trunkanimation')
        Citizen.Wait(2000)
        Config.CarTable[k].chopped = true
        TriggerServerEvent('elixir-chopshop:server:rewardplayer', Config.CarTable[k].anim)
        Citizen.Wait(1000)
    ...
    elseif Config.CarTable[k].anim == "hood" then
        SetVehicleDoorOpen(vehicle, Config.CarTable[k].destroy, false, true)
        TriggerEvent('elixir-chopshop:hoodanimation')
        Citizen.Wait(4000)
        SetVehicleDoorBroken(vehicle, Config.CarTable[k].destroy, true)
        Config.CarTable[k].chopped = true
        TriggerServerEvent('elixir-chopshop:server:rewardplayer', Config.CarTable[k].anim)
...
function UpdateBars(dist, plate)
    ...
    if dist < 1.0 then
        ...
        -- end
        if not mert then
            exports["elixir-thermite"]:thermiteminigame(6, 3, 2, 20,
                function()
                    TriggerEvents()
