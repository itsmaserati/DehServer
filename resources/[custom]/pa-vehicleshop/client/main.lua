...
Citizen.CreateThread(function()
    ...
    for k, v in pairs(Config.VehicleShops) do
        ...
        if v.Management.Enable then
            exports['pa-textui-2']:create3DTextUI("pa-vehicleshop-vehshop-" .. k, {
                ...
                triggerData = {
                    triggerName = "pa-vehicleshop:openSellingMenu:client",
            ...
            if v.EnableStocks then
                ...
                Citizen.CreateThread(function()
                    while true do
                        TriggerCallback('pa-vehicleshop:checkIsPlayerHasPerm:server', function(hasPerm)
                            if hasPerm then
                                isAdmin = true
                            end
                        end)
...
RegisterNetEvent('pa-vehicleshop:openSellingMenu:client', function(data)
    ...
    if sellMenuActive then
        ...
        if Config.VehicleShops[data.vehshopId].EnableStocks then
            ...
            TriggerCallback('pa-vehicleshop:getVehStock:server', function(stockData)
                if stockData ~= 0 then
...
--     Initialized = true
--     Citizen.CreateThread(function()
--         TriggerCallback('pa-vehicleshop:getShowroomData:server', function(showroomData)
...
--                     exports['pa-textui-2']:create3DTextUI("pa-vehicleshop-showroom-vehicle-" .. model .. "-spotId-" .. v.spotId, {
...
--                         theme = "green", 
--                         triggerData = {
--                             triggerName = "pa-vehicleshop:askAnEmployee:client",
--                             args = {}
--                         }
--                     })
--                 end
...
RegisterNetEvent('pa-vehicleshop:askAnEmployee:client', function()
...
RegisterNUICallback('callback', function(data)
    ...
    elseif data.action == "close" then
        ...
        elseif data.type == "showroomEdit" then
            ...
            TriggerServerEvent('pa-vehicleshop:updateShowroomVehicles:server', data.dealershipId, Config.VehicleShops[data.dealershipId].ShowroomVehicles)
        elseif data.type == "sellMenu" then
            TriggerEvent('pa-vehicleshop:openSellingMenu:client', {open = false})
        else
            SetNuiFocus(false, false)
        end
        if data.test == true and not testDriveState then
            if IsSpawnPointClear(vector3(currentCoords.testVehCoords.x, currentCoords.testVehCoords.y, currentCoords.testVehCoords.z), 2.5) then
                ...
                TriggerServerEvent('pa-vehicleshop:startTest:server', currentTestVehicleNetId)
    ...
    elseif data.action == "updateShowroomVehicle" then
        ...
        --TriggerServerEvent('pa-vehicleshop:deleteVehicleShowroom:server', data.dealershipId, data.spotId, data.model, props)
        TriggerServerEvent('pa-vehicleshop:deleteVehicleShowroom:server', data.dealershipId, data.spotId, data.model)
    ...
    elseif data.action == "createTextsOnPlayers" then
        ...
        if next(nearbyPlayers) ~= nil and next(nearbyPlayers) then
            ...
            for _, id in pairs(nearbyPlayers) do
                exports['pa-textui-2']:create3DTextUIOnPlayers("pa-vehicleshop-vehshop-players-" .. id, {
                    id = id,
                    displayDist = 5.0,
                    interactDist = 1.3,
                    ...
                    theme = "green", -- or red
                    triggerData = {
                        triggerName = "pa-vehicleshop:sendSellRequest:client",
            ...
            if next(nearbyPlayers) ~= nil and next(nearbyPlayers) then
                for _, id in pairs(nearbyPlayers) do
                    exports['pa-textui-2']:delete3DTextUIOnPlayers("pa-vehicleshop-vehshop-players-" .. id)
                end
            end
        else
    ...
    elseif data.action == "confirmPurchase" then
        TriggerServerEvent('pa-vehicleshop:buyVehicle:server', data.paymentType, data.model, Vehicles2[data.model].price, data.dealershipId, data.sender, Config.VehicleShops[data.dealershipId].Management.Job)
    elseif data.action == "addStockForAllVehicles" then
        ...
        TriggerServerEvent('pa-vehicleshop:updateDealershipStockData:server', data.dealershipId, stocks)
        taskActive = false
...
RegisterNetEvent('pa-vehicleshop:sendSellRequest:client', function(data)
    -- Delete All
    if next(nearbyPlayers) ~= nil and next(nearbyPlayers) then
        for _, id in pairs(nearbyPlayers) do
            exports['pa-textui-2']:delete3DTextUIOnPlayers("pa-vehicleshop-vehshop-players-" .. id)
    ...
    -- Add To Sender
    TriggerServerEvent('pa-vehicleshop:sendRequestText:server', data.sender, data.target, data.price, data.model, data.dealershipId)
    Citizen.Wait(500)
    taskActive = false
end)

currentSender = nil
RegisterNetEvent('pa-vehicleshop:sendRequestText:client', function(sender, price, model, dealershipId)
    currentSender = sender
    exports['pa-textui-2']:create3DTextUIOnPlayers("pa-vehicleshop-vehshop-players-confirm-purchase-" .. sender, {
        id = sender,
        displayDist = 5.0,
        interactDist = 1.3,
        enableKeyClick = true,
    ...
    taskActive = false
    Notify("Request timed out.", 7500, "error")
    exports['pa-textui-2']:delete3DTextUIOnPlayers("pa-vehicleshop-vehshop-players-confirm-purchase-" .. sender)
    currentSender = nil
end)

RegisterNetEvent('pa-vehicleshop:acceptPayment:client', function(data)
    Notify("Request accepted.", 7500, "success")
    exports['pa-textui-2']:delete3DTextUIOnPlayers("pa-vehicleshop-vehshop-players-confirm-purchase-" .. data.sender)
    currentSender = nil
    TriggerServerEvent('pa-vehicleshop:buyVehicle:server', "bank", data.model, tonumber(data.price), data.dealershipId, data.sender, Config.VehicleShops[data.dealershipId].Management.Job)
end)

RegisterNetEvent('pa-vehicleshop:declinePayment:client', function(data)
    Notify("Request declined.", 7500, "error")
    exports['pa-textui-2']:delete3DTextUIOnPlayers("pa-vehicleshop-vehshop-players-confirm-purchase-" .. data.sender)
    TriggerServerEvent('pa-vehicleshop:declinePayment:server', data.sender)
end)
    ...
    if GetCurrentResourceName() == resource then
        ...
        for k, v in pairs(Config.VehicleShops) do
            ...
            for a, b in pairs(v.ShowroomVehicles) do
                ...
                exports['pa-textui-2']:delete3DTextUI("pa-vehicleshop-showroom-vehicle-" .. model .. "-spotId-" .. b.spotId)
            end
            exports['pa-textui-2']:delete3DTextUI("pa-vehicleshop-vehshop-" .. k)
        end
        ...
        if currentSender then
            exports['pa-textui-2']:delete3DTextUIOnPlayers("pa-vehicleshop-vehshop-players-confirm-purchase-" .. currentSender)
        end
    end
end)
...
RegisterNetEvent('pa-vehicleshop:buyVehicle:client', function(model, plate, dealershipId)
    DoScreenFadeOut(500)
    openDealership(false)
    Citizen.Wait(1000)
...
RegisterNetEvent('pa-vehicleshop:finishTest:client', function()
    testDriveState = false
...
AddEventHandler('gameEventTriggered', function(event, data)
    if event == "CEventNetworkVehicleUndrivable" then
        ...
        if netId == currentTestVehicleNetId then
            TriggerEvent('pa-vehicleshop:finishTest:client')
            TriggerServerEvent('pa-vehicleshop:finishTest:server', netId)
        end
    end
    if event == "CEventNetworkEntityDamage" then
        local vehicle = data[1]
				...
				if closestDeliveryPoints.distance < closestDeliveryPoints.maxDist then
					if IsControlJustReleased(0, 38) then
                        ...
                        if VehToNet(veh) == VehToNet(currentTestVehicle) then
                            testDriveState = false
                            SendNUIMessage({action = "stopTestTimer"})
                            TriggerServerEvent('pa-vehicleshop:finishTest:server', VehToNet(currentTestVehicle))
...
RegisterNetEvent('pa-vehicleshop:deleteVehicle:client', function(netId)
...
--RegisterNetEvent('pa-vehicleshop:deleteVehicleShowroom:client', function(dealershipId, spotId, newModel, props)
RegisterNetEvent('pa-vehicleshop:deleteVehicleShowroom:client', function(dealershipId, spotId, newModel, createNew)
    -- Delete Old
