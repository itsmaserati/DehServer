    ...
    for k, v in pairs(Config.Shops) do
        for a, b in pairs(v.coords) do
            if Config.Text == 'textui2' then           
                ...
                exports['pa-textui-2']:create3DTextUI("pa-shop-" .. id, {
                    coords = coords,
                    displayDist = 5.0,
                    interactDist = 2.0,
                    enableKeyClick = true,
                ...
                for a, b in pairs(v.coords) do
                    ...
                    if dist <= 3 then
                        if Config.Text == 'textui1' then 
                            ...
                            exports["ac-text"]:openTextUi(item,b.coords,3)
    ...
    for k, v in pairs(Config.Shops) do
        ...
        for a, b in pairs(v.coords) do
            ...
            if Config.UseTextUI == false then
                ...
                -- exports['ac-textui']:AddTextUIPed(ObjToNet(b.ped), vector3(b.coords.x, b.coords.y, b.coords.z), 'E', 'Open Shop', 2.5)
            end
        end
    end
end
...
RegisterNUICallback("pay",function(data)
    TriggerServerEvent("ac-shop:pay", data)
end)
...
RegisterNetEvent('pa-shop:openShop')
AddEventHandler('pa-shop:openShop', function(data)
...
RegisterNUICallback('makePayment', function(data)
    TriggerServerEvent('ac-shop:makePayment', data.type, data.price, basket)
end)

RegisterNUICallback('makePaymentJob', function(data)
    TriggerServerEvent('ac-shop:makePayment', data.type, data.price, basketJob)
end)
